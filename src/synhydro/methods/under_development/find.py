...
from synhydro.core.base import Generator
...
class FIND(Generator):
    ...
    def preprocessing(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """
        ...
        data = data.resample("M").sum()  # Monthly totals
        ...
        if full_years_data.isnull().any().any():
            warnings.warn(
                "Missing values detected. Consider filling before generation."
            )
    ...
    def fit(
        ...
        **kwargs,
        ...
        durations, intensities, frequency, ssi_hist, drought_periods = (
            self._identify_droughts(primary_site, primary_site)
        )

        # Set target parameters
        nyears_hist = len(primary_site) // 12
        self.target_intensity = (
            target_intensity or np.mean(intensities) if len(intensities) > 0 else -1.0
        )
        self.target_duration = (
            target_duration or np.mean(durations) if len(durations) > 0 else 6
        )
        self.target_frequency = target_frequency or int(
            frequency / nyears_hist * self.nyears_generate
        )
        ...
        for i, site in enumerate(self.site_names):
            ...
            _, _, _, ssi, drought_periods = self._identify_droughts(
            ...
            non_drought_percentiles = (
                np.percentile(non_drought_ssi, [25, 50, 75])
                if len(non_drought_ssi) > 0
                else [0, 0, 1]
            )
    ...
    def generate(
        self, n_scenarios: int = 1, sites: Optional[list] = None, **kwargs
    ) -> pd.DataFrame:
        """
        ...
        for site in sites[1:]:
            ...
            for scenario in range(n_scenarios):
                primary_ssi = self._calculate_ssi(
                    self.obs_data.iloc[:, 0].values, primary_scenarios[scenario]
                )
    ...
    def _generate_single_site_scenario(self, site: str) -> np.ndarray:
        ...
        for temp_step in range(self.n_temp_reductions):
            ...
            cum_distributions = self._prepare_cumulative_distributions(
                obs_data, nmonths
            )
            ...
            for iteration in range(self.n_iterations):
                ...
                obj_candidate = self._calculate_objectives(
                    obs_data, candidate_series, site
                )
    ...
    def _generate_matched_site_scenario(
        self, site: str, target_ssi: np.ndarray
    ) -> np.ndarray:
    ...
    def _calculate_ssi(self, obs_ref: np.ndarray, obs_calc: np.ndarray) -> np.ndarray:
        ...
        for month in range(12):
            ...
            if self.distribution == "pearson3":
                ssi_month = self._calculate_ssi_pearson3(
                    ref_month_data, calc_month_data
                )
            else:  # gamma
    ...
    def _calculate_ssi_pearson3(
        self, ref_data: np.ndarray, calc_data: np.ndarray
    ) -> np.ndarray:
    ...
    def _calculate_l_moments(self, data: np.ndarray) -> Dict[str, float]:
        ...
        return {"lambda1": l1, "lambda2": l2, "lambda3": l3, "tau3": tau3}
    ...
    def _initialize_synthetic_series(
        self, obs_data: np.ndarray, monthly_stats: Dict
    ) -> np.ndarray:
        ...
        for i in range(n_months):
            ...
            if stats["type"] == "pearson3":
                ...
                empirical_cdf = np.arange(1, len(month_data) + 1) / (
                    len(month_data) + 1
                )
    ...
    def _prepare_cumulative_distributions(
        self, obs_data: np.ndarray, window_size: int
    ) -> Dict:
    ...
    def _knn_disaggregate(
        self, cum_value: float, n_months: int, historical: np.ndarray, start_month: int
    ) -> np.ndarray:
    ...
    def _calculate_objectives(
        self, obs_data: np.ndarray, synthetic: np.ndarray, site: str
    ) -> float:
        """Calculate weighted objective function for optimization."""
        # Identify droughts in synthetic series
        durations, intensities, frequency, ssi, drought_periods = (
            self._identify_droughts(obs_data, synthetic)
        )

        # Objective 1: Intensity
        if len(intensities) > 0:
            intensity_obj = np.mean(
                np.abs(
                    np.concatenate([intensities, [np.mean(intensities)]])
                    - self.target_intensity
                )
            )
        else:
        ...
        if len(durations) > 0:
            duration_obj = (
                np.mean(
                    np.abs(
                        np.concatenate([durations, [np.mean(durations)]])
                        - self.target_duration
                    )
                )
                / 100
        ...
        if frequency < self.target_frequency:
            ...
            frequency_obj = (
                np.mean(durations) if len(durations) > 0 else self.target_duration
            )
        else:
            frequency_obj = 0
    ...
    def _accept_swap(
        self, obj_current: float, obj_candidate: float, temperature: float
    ) -> bool:
