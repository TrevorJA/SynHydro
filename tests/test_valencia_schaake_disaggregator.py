...
class TestValenciaSchaakeInitialization:
    ...
    def test_initialization_default_params(self, sample_annual_series):
        ...
        disagg = ValenciaSchaakeDisaggregator(sample_annual_series)
...
class TestValenciaSchaakePreprocessing:
    ...
    def test_preprocessing_annual_dataframe(self, sample_annual_dataframe):
        ...
        disagg = ValenciaSchaakeDisaggregator(sample_annual_dataframe)
        disagg.preprocessing()
        assert disagg.is_preprocessed is True
...
class TestValenciaSchaakeFit:
    ...
    def test_fit_single_site(self, sample_annual_series):
        ...
        disagg = ValenciaSchaakeDisaggregator(sample_annual_series, transform='none')
        disagg.preprocessing()
        disagg.fit()
    ...
    def test_fit_multiple_sites(self, sample_annual_dataframe):
        ...
        disagg = ValenciaSchaakeDisaggregator(sample_annual_dataframe, transform='none')
        disagg.preprocessing()
        disagg.fit()
    ...
    def test_fit_with_log_transform(self, sample_annual_series):
        ...
        disagg = ValenciaSchaakeDisaggregator(sample_annual_series, transform='log')
        disagg.preprocessing()
        disagg.fit()
    ...
    def test_fit_with_boxcox_transform(self, sample_annual_series):
        ...
        disagg = ValenciaSchaakeDisaggregator(sample_annual_series, transform='boxcox')
        disagg.preprocessing()
        disagg.fit()
    ...
    def test_fit_without_transform(self, sample_annual_series):
        ...
        assert len(disagg.transform_params_) == 0 or disagg.transform_params_.get('type') == 'none'
...
class TestValenciaSchaakeDisaggregation:
    ...
    def test_disaggregate_ensemble(self, sample_annual_series):
        ...
        for realization in range(2):
            data = {
                'site_1': np.random.lognormal(mean=7.0, sigma=0.4, size=len(dates))
            }
    ...
    def test_disaggregation_preserves_sum(self, sample_annual_series):
        ...
        disagg = ValenciaSchaakeDisaggregator(
            sample_annual_series,
            transform='log',
            conservation_method='proportional'
        )
        disagg.preprocessing()
        disagg.fit()
...
class TestValenciaSchaakeEdgeCases:
    ...
    def test_disaggregate_before_fit_raises_error(self, sample_monthly_ensemble, sample_annual_series):
    ...
    def test_small_sample_size(self):
        ...
        disagg = ValenciaSchaakeDisaggregator(Q_obs, transform='none')
        disagg.preprocessing()
    ...
    def test_zero_flows_handled(self):
        ...
        values = np.array([1000.0, 0.1, 1100.0, 500.0, 1200.0, 300.0] * 12)  # 6 years of monthly data
        Q_obs = pd.Series(values[:len(dates)], index=dates, name='site_1')

        disagg = ValenciaSchaakeDisaggregator(Q_obs, transform='log')
        disagg.preprocessing()
        disagg.fit()
        assert disagg.is_fitted
